...
const CITY_OPTIONS: CityIdentity[] = [
  { city: 'Sargodha', district: 'SARGODHA', tehsil: 'SARGODHA', color: 'text-emerald-500', gradient: 'from-emerald-500/20 to-emerald-500/5', label: 'SGD', subtitle: 'Headquarters' },
  { city: 'Bhalwal', district: 'SARGODHA', tehsil: 'BHALWAL', color: 'text-blue-500', gradient: 'from-blue-500/20 to-blue-500/5', label: 'BHL', subtitle: 'Branch Office' },
  { city: 'Khushab', district: 'KHUSHAB', tehsil: 'KHUSHAB', color: 'text-amber-500', gradient: 'from-amber-500/20 to-amber-500/5', label: 'KHB', subtitle: 'Regional Office' },
...
function CityAvatar({ identity, size }: { identity: CityIdentity; size: 'sm' | 'md' }) {
  return (
    <div className={cn(
      ...
      size === 'sm' ? 'h-8 w-8 text-[9px]' : 'h-9 w-9 text-[10px]',
    ...
    <div className="relative px-3 py-2">
      ...
      >
        ...
        <div className="flex flex-col items-start overflow-hidden flex-1 text-left">
          <span className="text-[13px] font-bold text-sidebar-foreground leading-tight truncate w-full">
      ...
      {open && (
        <>
          ...
          <div className="absolute left-3 right-3 top-full z-50 mt-1 bg-popover border border-border rounded-lg shadow-lg p-1.5">
            <div className="px-2 py-1.5 text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest mb-1 border-b border-border/40">
              Switch City
            </div>
            {CITY_OPTIONS.map((opt) => (
              ...
              >
                ...
                <div className="flex flex-col items-start overflow-hidden flex-1">
                  ...
                  <span className="text-[10px] text-muted-foreground/60 font-medium leading-none mt-0.5">
                    {opt.subtitle}
                  </span>
                </div>
                {current.city === opt.city && (
