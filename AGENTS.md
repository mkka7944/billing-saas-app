...
7. **Polling is admin-controllable** — hooks read `useSettings()` for `live_polling_enabled` / `live_poll_interval` (default 60s) and `notifications_polling_enabled` / `notifications_poll_interval` (default 120s). Stored in `app_settings` DB table. Admin configures via Settings → General → Data Usage. Non-admins see read-only summary.
8. **Context-aware refresh** — use `refreshCurrentPage(pathname, queryClient)` from `src/lib/queries/refresh.ts` instead of blanket `queryClient.invalidateQueries()`. It invokes only the query keys relevant to the current route (`/deliver`, `/map`, `/assignments`, or all). Wired into AppHeader refresh button, FloatingActions hexagon Refresh button, and DesktopFilterBar `handleUpdate`.
9. **Mutate → invalidate pattern**: Every mutation must invalidate the query keys it affects. Invalidating the prefix (e.g., `['assignment-totals']`) invalidates all sub-keys (e.g., `['assignment-totals', month, district]`).
